...
class UpdateAvatar:
    ...
    async def execute(self, user_id: str, file: Any) -> UserPublicDto:
        ...
        if user.avatar_url:
            try:
                await self.storage_service.delete_profile_photo(user_id, ProfilePhotoType.AVATAR)
            except Exception:
        ...
        url = await self.storage_service.upload_profile_photo(file, user_id, ProfilePhotoType.AVATAR)
        user.avatar_url = url
